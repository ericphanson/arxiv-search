...
time find . -type f -name "tsconfig.json" -not -path "*/node_modules/*" -execdir tsc \;
echo "(2/4) Preparing package and uploading to S3 bucket $CODE_BUCKET..."
time aws cloudformation package --template-file template.cf.json --output-template-file output.yaml --s3-bucket $CODE_BUCKET
echo "(3/4) Deploying to AWS CloudFormation..."
time aws cloudformation deploy --template-file output.yaml --stack-name arxiv-search-stack --capabilities CAPABILITY_IAM
rm output.yaml
