...
exports.handler = (event, context, callback) => {
    ...
    function process_record (record)  {
        ...
        for (let i in status_urls) {
            ...
            if (get && num_urls_so_far <= MAX_NUM_TO_DOWNLOAD ) {
                getSignedURL(record.idvv.S, sk).then( (signed_url) => {
                      
                    update_db(record.idvv.S, sk).then( (result) => {
                    fetch_url = record[uk].S
                    list_of_download_params.push( {"fetch" : fetch_url , "submit" :  signed_url} )
                    num_urls_so_far++ 
                    } ).catch((err) => {
    ...
    //  ??? need to make sure this doesn't fire until all the async processing is done...
